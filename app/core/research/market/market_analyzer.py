...
class MarketIndex(BaseModel):
    ...
    class Config:
        json_encoders = {
            float: lambda v: round(v, 2)  # 浮点数保留2位小数
        }
...
class MarketOverview(BaseModel):
    ...
    top_sectors: List[SectorInfo] = []  # 涨幅前5板块
    bottom_sectors: List[SectorInfo] = []  # 跌幅前5板块
...
class MarketAnalyzer:
    ...
    def __init__(self, search_service: Optional[Any] = None, analyzer=None):
        """
        初始化大盘分析器
